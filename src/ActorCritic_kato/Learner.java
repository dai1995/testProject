...
public class Learner {
	...
	public void output(double x[],double y){//新しい学習サンプルを学習するかの判定？
		double deruta = 0;
		Deruta de = new Deruta(this.NumberofKernels,this.NumberofInput,this.sigma);//sigma 削除の度合い 大きいほど使わないサンプルが増える
		this.input = x;
		double output = calcurateoutput(this.centers,answer);
		double cumerr = err(output,y);
		double hantei = hantei(this.centers,this.input);//単純に学習済みかの判定 
		if(this.count <= this.NumberofKernels){
			this.count++;
			if(hantei == 0){  //hanteiの値が0ならば学習。0でなければ入力と同じものが学習されている。
				learn(x,y);//先に学習してδ<しきい値ならばここで学習したカーネルを初期化新しい入力の削除
				if(this.Kernelcount>=3){
					//冗長度の計算
					deruta = de.deruta(this.input,this.centers,this.Kernelcount);//this.center：学習したカーネルの中心位置getT() 
...
/*
	...
	public double hantei(double centers[][],double input[]){	//学習するかの判定
		...
		for(int i=0; i<this.Kernelcount;i++){
			double dis=0;
			for(int j=0; j<this.NumberofInput; j++){
				dis += Math.pow(this.input[j]-this.centers[index][j], 2);//2乗誤差 誤差が0ならすでに学習済み
			}
