...
import KernelPerceptron.Kernel4LRFU;
import KernelPerceptron.KernelLeastSquareLRFU;
...
public class ActorCriticKernel {
	Status currentStatus;//現在の状態
	Status futureStatus;//一時刻先の状態
	Actor actor;
	Critic critic;
	public ActorCriticKernel(Node nd_value_function, Node nd_actor){
	...
	public void initLearning(double[] inputs, double[] outputs){
		...
		this.actor.LGRNNactor.learning(inputs, outputs, 1D);
	}
	public double[] getAction(double[] inputs, int count){
	...
	public class Status{
		...
		double ValueFunction, action[], inputs[], drnd[];		
		//Stasusをコンソールに表示---------------------------------------------------------------------------------------
		public void display() {
	...
	static class Actor{
		final boolean DEBUG = true;
		private KernelLeastSquareLRFU LGRNNactor;//Actorで使用するLGRNN
		...
		public Actor(Node nd){
			this.LGRNNactor = new KernelLeastSquareLRFU(nd);	
		...
		public void learning(Status currentStatus, double td_error, double UtilityFunction){
			...
			System.out.println("max = " + this.max_variance);
			if(td_error > 0){
				//平均と分散を更新
...
//				this.actor_output[1] += beta * (currentStatus.drnd[1] - this.actor_output[1]);
				...
				switch ((int)currentStatus.action[0]){
					case 0:
		...
		public double[] getAction(double inputs[], Status currentStatus, int count){
			double[] result = new double[this.LGRNNactor.getOutputSize() / 2];
			int mean, variance;
...
//			System.out.println("AgentPointY = " + inputs[1]);
			...
			result[0] = ActionGenerator(this.actor_output[0], this.actor_output[1], this.actor_output[2], this.actor_output[3], currentStatus);
	...
	static class Critic{
		final boolean DEBUG = true;
		private KernelLeastSquareLRFU LGRNNcritic;//Critc用のLGNN
		
		public Critic(Node nd){
			this.LGRNNcritic = new KernelLeastSquareLRFU(nd);
