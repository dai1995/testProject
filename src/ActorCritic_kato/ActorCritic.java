...
public class ActorCritic {
	...
	final boolean IsRiskSensitiveRL=true;// If you want to execute RiskSensitiveRL, set true.
	Actor actor;
	Critic critic;
	public ActorCritic(Node nd_value_function, Node nd_actor){
		this.currentStatus = null;
		this.futureStatus = null;
		this.actor = new Actor(nd_actor, this.IsRiskSensitiveRL);
		this.critic = new Critic(nd_value_function, this.IsRiskSensitiveRL);
	}
...
//			}
	...
	static class Actor{
		...
		private boolean IsRiskSensitiveRL = false;
		//Node:prameterファイルからの読み込みに使う
		public Actor(Node nd, boolean IsRiskSensitiveRL){
			this.LGRNNactor = new LimitedGRNNLRFU(nd);
			this.IsRiskSensitiveRL = IsRiskSensitiveRL;
		}
...
//			System.out.println("AgentPointY = " + inputs[1]);
	...
	static class Critic{
		...
		private boolean IsRiskSensitiveRL = false;
		public Critic(Node nd, boolean IsRiskSensitiveRL){
