...
//import java.util.Scanner;
///LRFUを組み込んだLGRNN、近藤君の作ったLGRNNのCプログラムは
//高速計算のための簡易アルゴリズムを用いていたため、実際のLGRNN-orgに負けてしまう。
//そこで条件を揃えるためにJavaで再実装
...
public class LimitedGRNNLRFU extends grnnLinearDependency implements actor_critic {
	...
	Scanner keyScanner;
	public LimitedGRNNLRFU(Node nd) {
		super(nd);
	...
	void init_parameters(Node nd) {
		...
		this.lambda = lgp.getLambda();
		this.delta_threshold = lgp.getDelta_threshold();
		System.out.println("LimitedGRNNLRFU.init_parameters(): IsCumulatieveErrorEstimation = " + this.isCumulativeErrorEstimation);

		System.out.println(UseIgnore + " " + UseModify + " " + UsePrune + " "
	...
	public LgrnnLearningStatus learning(double inputs[], double outputs[]) {
		try {
			return this.LGRNN_learning(inputs, outputs, this.UpperLimitOfHiddenUnits,
					this.ImportanceWeight4NewSample, this.delta_threshold, this.UsePseudoInverse);
...
//	public LgrnnLearningStatus learning(double inputs[], double outputs[], double delta_threshold) {
//		try {
//			return this.LGRNN_learning(inputs, outputs, this.UpperLimitOfHiddenUnits,
//					this.ImportanceWeight4NewSample, delta_threshold, this.UsePseudoInverse);
//		} catch (MatrixException e) {
//			// TODO Auto-generated catch block
//			e.printStackTrace();
//		}
...
//	}
	...
	Cell IsExistTheSameKernel(ArrayList<Cell> hidden_units, double center[]) {
		double diff;
		CellLRFU NearestCell = (CellLRFU) this.getNearestUnit(center);
		if (NearestCell==null) return null;
		if (NearestCell.getActualDistance() == 0.0) {
			return NearestCell;
		}
	...
	LgrnnLearningStatus LGRNN_learning(double inputs[], double outputs[],
			int UpperLimitOfHiddenUnits, double importance_weight, double delta_threshold, boolean UsePseudoInverse)
			throws MatrixException {
		CellLRFU each_cell, new_cell;
		...
		double[] error;
		error = VectorFunctions.diff(outputs, this.calculate_outputs(inputs, true, importance_weight));
		...
		if (this.hidden_units.size() < UpperLimitOfHiddenUnits) {// only adding
																	// new units
			this.Log("LGRNN_learning() append new unit");
			new_cell = new CellLRFU(this.NumberOfInputs, this.NumberOfOutputs, this.lambda);
			...
			this.Log("LGRNN_learning() bounded!");
			UselessCell uselesscell_LD = this.getMostUselessCellLD(UsePseudoInverse, delta_threshold);
			...
			if (this.UseEditing && eloss.mag_editing < eloss.mag_ignore) {
				...
				// get Minimum interference
				if (uselesscell_LD.IsRedundant && this.UseSubstitution
				...
				case LgrnnLearningStatus.SUBSTITUTION:
					...
					System.out
							.println("========LRFU substitution & replace==========");
					for (Cell cell : this.hidden_units) {
						each_cell = (CellLRFU)cell;
				...
				case LgrnnLearningStatus.PRUNING:
					System.out.println("======== LRFU replace ==========");
				...
				case LgrnnLearningStatus.MODIFY:
					System.out.println("======== LRFU Modify=========");
					int index2 = 0;
					// double[] A = new double[this.NumberOfOutputs];
					double total_a =0; 
				...
				case LgrnnLearningStatus.IGNORE:
					// do nothing!
					//System.out.println("=====LRFU IGNORE=====");
					if (eloss.mag_ignore > delta_threshold) {
						double MinCb = Double.MAX_VALUE;
						CellLRFU mostLRFU = null;
						for (Cell each_kernel : this.hidden_units) {
							each_cell = (CellLRFU)each_kernel;
							if (MinCb > each_cell.getCb()) {
								
								MinCb = each_cell.getCb();
								mostLRFU = each_cell;
								
							}
							this.Log("LGRNN_learning() MinCb=" + MinCb);
						}
						//mostLRFに(inputs, outputs)を学習させる。
						if (mostLRFU != null) {
							mostLRFU.learn(inputs, outputs, 1);
							this.Log("LGRNN_learning() mostLRFU Cb=" + mostLRFU.getCb());
							mostLRFU.Initialization(importance_weight);
							//mostLRFU.Initialization(100D);
						}
					}
					break;
				}
	...
	public void Log(String log) {
		if (this.DEBUG)
		System.out.println("LimitedGRNNLRFU." + log);
	}
	void LogAlwaysDisplay(String log) {
		System.out.println("LimitedGRNNLRFU." + log);
	}
	void OptimizeSigma() {
		...
		Log("OptimizeSigma(): simga_opt=" + sigma_opt + " mean_distance is " + mean_distance);
	...
	double appropreate_sigma(double min_sigma, double max_sigma) {
		...
		Log("appropreate_sigma(): upper_error=" + upper_error + ", lower_error=" + lower_error);
	...
	double appropreate_sigma(double min_sigma, double max_sigma, double step_size) {
		...
		for (double s=min_sigma; s<=max_sigma; s+=step_size) {
			...
			Log("appropreate_sigma(min_sigma,max_sigma,step_size): I'm tring sigma=" +s+ " error=" + each_error);
	...
	double EstimateSigma(double sigma) {
		...
		for (Cell cell : this.hidden_units) {
			...
			output = this.calculate_outputs(target_unit.getT(), false, 0);
	...
	ExpectedLoss EstimateExpectedLoss(double inputs[], double outputs[],
			double importance_weight, UselessCell uselesscell_LD,
			UselessCell uselesscell_EDIT, dependency dep_new)
			throws MatrixException {
		ExpectedLoss e = new ExpectedLoss();
		// double delta_new_sample;
		// initialize data
		this.Log("EstimateExpectedLoss()");
		e.mag_editing = e.mag_ignore = e.mag_modification = e.mag_pruning = 0;

		// estimate expected loss
		//System.out.print("LimitedGRNN:EstimateExpectedLoss():");
		if (this.UseSubstitution) {
			e.mag_substitution = this.affectSubstituteRevised(
					uselesscell_LD.minimum_delta, outputs,
					uselesscell_LD.alpha, uselesscell_LD.target_cell);
			System.out.print(" Affect substition & modify="
					+ e.mag_substitution);
		} else
			e.mag_substitution = Double.MAX_VALUE;
		if (this.UsePrune) {
			if (uselesscell_LD.IsRedundant) {
				e.mag_pruning = this.affectJustPruningRevised(uselesscell_LD.target_cell);
			}else{
				e.mag_pruning = this.affectJustPruningLRFU(uselesscell_LD.target_cell, uselesscell_LD.minimum_delta);
			}
		...
		if (this.UseModify) {
			e.mag_modification = this.affectModifyRevised(dep_new.delta, inputs, outputs,
	...
	UselessCell getMostUselessCellLD(boolean UsePseudoInverse, double delta_threshold) {
		CellLRFU each_cell;
		dependency dep;
		double delta;
		this.Log("getMostUselessCellLD()");
		UselessCell uselesscell = new UselessCell();
		uselesscell.minimum_delta = Double.MAX_VALUE;
		uselesscell.target_cell = null;
		this.Log("getMostUselessCellLD() start");
		for (Cell cell : this.hidden_units) {
			each_cell = (CellLRFU)cell;
			try {
				dep = this.Calculate_LinearDependency(each_cell, UsePseudoInverse);
				if (dep.delta < 0) {
					this.Log("getMostUselessCellLD() delta = " + dep.delta);
					dep.delta = 0;
				}
				each_cell.setDelta(dep.delta);
				each_cell.setMatrixAlpha(dep.alpha);
			...
			// System.out.print("LimitedGRNN:getMostUselessCellLD():delta=" +
			// delta);
			// delta = dep.delta *
			...
			if (delta < uselesscell.minimum_delta) {
				...
				uselesscell.IsRedundant = true;
			}
		}
		/*
		 * following codes are Yamauchi's original idea. This code might be removed by Kondo.
		 */
		/*if (uselesscell.target_cell != null) {
			double MinCb = Double.MAX_VALUE;
			if (uselesscell.minimum_delta < delta_threshold) {
				this.Log("getMostUselessCellLD() Redundant!" );
				return uselesscell;
			}else{
				for (Cell each_kernel : this.hidden_units) {
					each_cell = (CellLRFU)each_kernel;
					if (MinCb > each_cell.getCb()) {
						this.Log("getMostUselessCellLD() MinCb=" + MinCb);
						MinCb = each_cell.getCb();
						uselesscell.target_cell = each_cell;
						uselesscell.minimum_delta = each_cell.getCb();
						uselesscell.alpha = each_cell.getMatrixAlpha();
						uselesscell.IsRedundant = false;
					}
				}
				return uselesscell;
			}
	...
	/*
	 * Get the Lesast frequently and recently used cell.
	 * This method is used in LGRNN_learning().
	 */
	CellLRFU getLRFUCell() {
		CellLRFU each_cell = null;
		double MinCb = Double.MAX_VALUE;
		
		CellLRFU targetLRFUCell = null;
		for (Cell each_kernel : this.hidden_units) {
			each_cell = (CellLRFU)each_kernel;
			if (MinCb > each_cell.getCb()) {
				this.Log("getLRFUCell() MinCb=" + MinCb);
				MinCb = each_cell.getCb();
				targetLRFUCell = (CellLRFU)each_kernel;
			}
		}
		return targetLRFUCell;
	}
	
	/*
	 * (non-Javadoc)
	 * @see mgrnn.grnnLinearDependency#calculate_outputs(double[])
	 * Override
	 *  input[] : input vector
	 *  UpdateLRFUStatus: If you want to update the status parameters of CellLRFU, set this flag true.
	 */
	public double[] calculate_outputs(double input[], boolean UpdateLRFUStatus, double ImportanceWeight) {
		double sum_output=0D;
		double[] each_output;
		double[] sum_outputs = new double[this.NumberOfOutputs];
		double min_distance = Double.MAX_VALUE;
		Cell NearestCell = null;
		//System.out.printf("Number of Outputs = %d\n", this.NumberOfOutputs );
		if (this.hidden_units.size()>0) {
			for (Cell cell_obj: this.hidden_units) {
				if (!cell_obj.is_check()) {//checkが付いていないならば 
					...
					if (cell_obj.getActualDistance() < min_distance) {
						min_distance = cell_obj.getActualDistance();
						NearestCell = cell_obj;
					}
			...
			if (UpdateLRFUStatus) {
				...
				CellLRFU NearestCellLRFU = (CellLRFU)NearestCell;//cast: Cell -> CellLRFU
				NearestCellLRFU.SetActivate(ImportanceWeight);//modify the status parameters.
				//NearestCellLRFU.SetActivate(1D);
				for (Cell each_cell : this.hidden_units) {
					CellLRFU each_cellLRFU = (CellLRFU)each_cell;
	...
	dependency Calculate_LinearDependency(CellLRFU target_cell, boolean UsePseudoInverse)
			throws MatrixException {
		...
		//this.Log("Calculate_LinearDependency():delta is " + dep.delta);
		return dep;
	}
	...
	double affectSubstitute(double delta, double target_output[],
			MatrixObj alpha, CellLRFU target_cell) {
		final boolean NewPruningErr=false;
		...
		for (Cell cell: this.hidden_units) {
			if (cell != target_cell) {
				...
				for (int o = 0; o < this.NumberOfOutputs; o++) {
					...
					//以下はProjectionして値が変わることを勘案した距離を求めるもの。
		...
		for (int j = 0; j < this.NumberOfOutputs; j++) {
			if (NewPruningErr){ 
				...
				/*loss_of_target += Math.pow(target_cell.ActualWeight()[0], 2D)
					* delta * target_cell.getAlpha();*/
				loss_of_target += target_cell.getNumberOfLearnedSamples() *
						Math.pow((target_cell.ActualWeight()[j] - ClosestUnit.get_normalized_t_alpha()[j]), 2D) *
						delta;
			}
		}
		if (isNegative) {
	...
	double affectSubstituteRevised(double delta, double target_output[],
			MatrixObj alpha, CellLinearDependency target_cell) {
		final boolean NewPruningErr=false;
		CellLinearDependency ClosestUnit = this.getNearestUnit(target_cell.getT(), target_cell);
		int index_of_closest_unit = -1;
		double results[] = new double[this.NumberOfOutputs];
		...
		double loss_of_target = 0D;
		double loss_of_forget = 0D;
		for (int j = 0; j < this.NumberOfOutputs; j++) {
			if (NewPruningErr){ 
				loss_of_target += target_cell.getNumberOfLearnedSamples() * 
						Math.pow((target_cell.ActualWeight()[j] * ClosestUnit.alpha - 
			...
			loss_of_forget += results[j];
		}
		if (isNegative) {
			System.err.println("LimitedGRNN.affectSubstitute(): isNegative!");
			return Double.MAX_VALUE;
		}
		else return loss_of_forget + loss_of_target;
		// return 0;
	}// affectSubstitute()	
	...
	double affectModify(double delta, double target_input[], double target_output[], 
				MatrixObj alpha, double importance_weight) {
		final boolean NewPruningErr=false;
		double results[] = new double[this.NumberOfOutputs];
		CellLinearDependency each_cell;
		...
		for (Cell cell : this.hidden_units) {
			...
			for (int o = 0; o < this.NumberOfOutputs; o++) {
				...
				results[o] += each_cell.getNumberOfLearnedSamples()
						* Math.pow(
								alpha.getData(cell_index, 0)
										* (target_output[o] - each_cell
												.ActualWeight()[o])
										/ (each_cell.getAlpha() + alpha.getData(
												cell_index, 0)), 2D); // affect
																		// of
																		// substitution
				if (sum_alpha < 0) isNegative = true;
			}
			...
			if (NewPruningErr) {
				...
				/*results[j] += importance_weight * delta
					* Math.pow(target_output[j], 2D); // affect of remained
														// error*/
				results[j] += importance_weight * delta *
						Math.pow(target_output[j] - ClosestUnit.get_normalized_t_alpha()[j], 2D);
	...
	double affectModifyRevised(double delta, double target_input[], double target_output[], 
			MatrixObj alpha, double importance_weight) {
	final boolean NewPruningErr=false;
	double results[] = new double[this.NumberOfOutputs];
	CellLinearDependency each_cell;
	...
	double affectJustPruning(CellLinearDependency target_cell) {
		...
		for (int o = 0; o < this.NumberOfOutputs; o++) {
			affect[o] += target_cell.getNumberOfLearnedSamples()
					* Math.pow((target_cell.ActualWeight()[o] - ClosestUnit
							.ActualWeight()[o]), 2);
		}
		return VectorFunctions.getSqureNorm(affect);
		// return 0;
	...
	//こっちが正しい
	double affectJustPruningRevised(CellLinearDependency target_cell) {
		double[] affect = new double[this.NumberOfOutputs];
		CellLinearDependency ClosestUnit;
		ClosestUnit = (CellLinearDependency)this.getNearestUnit(target_cell);
		for (int o = 0; o < this.NumberOfOutputs; o++) {
			affect[o] =(target_cell.ActualWeight()[o] - ClosestUnit
							.ActualWeight()[o]);
		}
		return target_cell.getNumberOfLearnedSamples() * VectorFunctions.getSqureNorm(affect);
		// return 0;
	}// affectJustPruning()	
	
	double affectJustPruningLRFU(CellLinearDependency target_cell, double prob) {
		double[] affect = new double[this.NumberOfOutputs];
		CellLinearDependency ClosestUnit;
		ClosestUnit = (CellLinearDependency)this.getNearestUnit(target_cell);
		for (int o = 0; o < this.NumberOfOutputs; o++) {
		...
		return prob * VectorFunctions.getSqureNorm(affect);
		// return 0;
	}// affectJustPruning()		
	//EstimateExpectedLossに雇われるメソッド
	//学習しない場合の影響
	double affectIgnore(double inputs[], double outputs[],
			double importance_weight) {
		double activity[] = this.calculate_outputs(inputs, false, 0);
		return importance_weight
				* VectorFunctions.getSqureNorm(VectorFunctions.diff(outputs,
						activity));
	...
	CellLRFU getNearestUnit(double x[],
			Cell ExceptCell) {
		CellLRFU cellobj, nearest_cellobj = null;
		double min_distance = Double.MAX_VALUE;
		this.calculate_outputs(x, false, 0);
		for (Cell cell : this.hidden_units) {
			cellobj = (CellLRFU)cell;
	...
	public Cell getNearestUnit(double inputs[]) {
		CellLRFU nearest_cellobj=null;
		double min_distance = Double.MAX_VALUE;
		this.calculate_outputs(inputs, false, 0);
		for (Cell cellobj : this.hidden_units) {
			if (cellobj.getActualDistance() < min_distance ) {
				nearest_cellobj = (CellLRFU)cellobj;
				min_distance = cellobj.getActualDistance();
			}
		}
		return nearest_cellobj;
	...
	void Aggregation(CellLinearDependency target_cell,
			CellLinearDependency nearest_unit) {
		double[] new_center;
		double new_sigma;
		double[] new_output;
		double new_alpha, new_t_alpha[];
		new_center = VectorFunctions.add(target_cell.getT(),
				nearest_unit.getT());
		...
		new_t_alpha = VectorFunctions.add(target_cell.getT_alpha(),
				nearest_unit.getT_alpha());
	...
	public void DeleteUnit(){
		this.hidden_units.remove(this.hidden_units.size() - 1);
	}
	...
	class UselessCell {
		CellLRFU target_cell;
		MatrixObj alpha;
		double minimum_delta;
		boolean IsRedundant = false;//選ばれたユニットが冗長なので選ばれたのかLRFUで選ばれたのかの情報。
	...
	class ExpectedLoss {
		double mag_substitution, mag_pruning, mag_ignore, mag_modification,
				mag_editing;
	}
	...
	@Override
	public LgrnnLearningStatus learning(double[] inputs,
			double[] outputs, double UtilityFunction) {
		LgrnnLearningStatus LLS=null;
		// TODO Auto-generated method stub
		try {
			LLS = this.LGRNN_learning(inputs, outputs, this.UpperLimitOfHiddenUnits,
					UtilityFunction, this.delta_threshold, this.UsePseudoInverse);
		} catch (MatrixException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
